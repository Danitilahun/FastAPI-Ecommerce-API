...
def create_category(client: TestClient, authenticated_admin_user: dict):
    ...
    print("authenticated_admin_user", authenticated_admin_user)
...
def create_product(client: TestClient, authenticated_admin_user: dict, create_category: dict):
    ...
    assert response.status_code == 201  # Ensure the product was created successfully
    return response.json()["data"]  # Return created product data